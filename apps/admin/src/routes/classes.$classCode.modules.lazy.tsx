...
function Lessons({}) {
  ...
  const { toggleDialog: toggleDeleteDialog, dialogIsOpen: deleteDialogIsOpen } =
    useDialog();
  const { toggleDialog: toggleCreateDialog, dialogIsOpen: createDialogIsOpen } =
    useDialog();
  const { dialogIsOpen: refreshFlag, toggleDialog: toggleRefreshFlag } =
    useDialog();
  const { dialogIsOpen: filterIsShown, toggleDialog: toggleFiltersAreShown } =
    useDialog();
  const {
    isLoading: deleteIsLoading,
    resetLoading: resetDeleteIsLoading,
    toggleLoading: toggleDeleteIsloading,
  } = useLoading();

  const { changeFilter, filter, filterChangedFlag, filterOptions } =
    useModuleStateFilter();

  const { compositeFilterFlag, manuallyToggleCompositeFilterFlag } =
    useCompositeFilterFlag([refreshFlag, filterChangedFlag]);

  const {
    isLoading: modulesIsLoading,
    modules,
    count: modulesCount,
  } = useModules(compositeFilterFlag, 50, 0, classCode, {
    ...filter,
    user,
  });
  ...
  const {
    ...
    resetSelected,
  } = useSpecificEntity<IModuleDoc>();

  function handleCreateModule(values: { title: string }) {
    abstractAuthenticatedRequest(
      "post",
      "/api/v1/modules",
      {
          ...
          {(form) => {
            return (
              <>
                ...
                <div className="flex flex-shrink-0 gap-2 py-6 overflow-x-auto">
                  {filesHook.files.map((resource, idx) => {
                    return (
                      <a
                        className="flex max-w-[200px] justify-between gap-2 p-1 rounded-sm bg-blue-600/20"
                        target="_blank"
                        href={resource.link}
                        key={idx}
                      >
                        ...
                        <XMarkIcon
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            filesHook.removeSpecificFile(`${resource._id}` );
                          }}
                        />
                      </a>
                    );
                ...
                <div className="flex justify-end gap-4 mt-3">
                  <Button
                    className="!h-[35px] w-24 self-start"
      ...
      <div className="flex flex-col w-full h-[calc(100vh-6rem)] sm:h-full flex-1">
        <div className="mt-2 flex h-fit justify-between items-center">
          ...
          <Link to={`/classes/${classCode}/modules/create`}>
            ...
            >
              {" "}
              <PlusIcon className="inline w-4 mr-1" /> Create{" "}
              <span className="hidden sm:inline">&nbsp;a Module</span>
            </Button>
          </Link>
        </div>
        <div className="w-full mt-3 flex flex-wrap gap-4">
          ...
          <div className="flex flex-col gap-2 justify-end">
            <Button
              className="!h-[35px] px-2"
              variant="outline"
              onClick={manuallyToggleCompositeFilterFlag}
            >
              {" "}
          ...
          <div className="bg-white w-full overflow-auto h-full flex flex-col rounded">
            ...
            {!modulesIsLoading &&
              modules.map((module, idx) => {
                return (
                  <Link
                    to={`/classes/${classCode}/modules/${module.code}`}
                    key={idx}
                  ...
                  >
                    ...
                    <div className="flex gap-4 items-center font-light">
                      ...
                      <span className="w-[100px] flex gap-2 justify-end">
                        <Link
                          to={`/classes/${classCode}/modules/${module.code}/update`}
                          className="grid place-items-center w-7 h-7 rounded-full bg-blue-50 hover:bg-blue-200 cursor-pointer duration-150"
                        >
        ...
        <div className="flex justify-end text-blue-700 mt-2 pb-2">
          <p>
            ...
            <span className="font-semibold">{modulesCount}</span>
          </p>
        </div>
      </div>
    </>
...
const createModuleValidationSchema = Yup.object({
  title: Yup.string()
    ...
    .min(4, "Module is too short")
    .max(256, "Module name is too long"),
});
